...
document.getElementById('root').innerHTML = categories.map((item) => {
    return (
      `<div>
      <button id="bou" style="position: absolute; top: 66%; z-index:2000; color:white; background-color:black; width:10%; height:5%; cursor: pointer;" class="add-bag-btn" onclick='addtocart(${i++})'>Ajouter au panier</button></div> 
       `
    );
  }).join('');
